...
class ForecastFragment : Fragment(), OnChartValueSelectedListener {
    ...
    private fun setLineChart(stockInfo: StockInfo, predict: Predict?) {
        ...
        for (i in 0..<90) {
            ...
            values.add(stockInfo.takeLast(90)[i].Date.replace("2023-", "").replace("2024-",""))
        }
        repeat(30) {
            values.add("")
        }
    ...
    override fun onResume() {
        super.onResume()
        val arrayAdapter = ArrayAdapter(requireContext(), R.layout.dropdown_item, companyData.map { it.ticker })
